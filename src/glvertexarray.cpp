...
namespace fbox
{
	...
	FBOXAPI void GlVertexArray::create()
	{
		...
		unsigned int components, attribute attributes, const unsigned int* strides, const unsigned int* offsets)
	{
		...
		this->_vertices = new GlBuffer(GlBuffer::TARGET_ARRAY, GlBuffer::USAGE_STATIC_DRAW);
		...
		this->_indices = new GlBuffer(GlBuffer::TARGET_ELEMENT_ARRAY, GlBuffer::USAGE_STATIC_DRAW);
	...
	FBOXAPI void GlVertexArray::create(Mesh::Shape* shape)
	{
		if (shape == 0 || shape->isEmpty() || this->_program == 0)
		{
			return;
		}
	...
	FBOXAPI void GlVertexArray::release()
	{
		this->_elements = 0;
		if (this->_handle != 0)
		{
	...
	FBOXAPI void GlVertexArray::draw()
	{
	...
	FBOXAPI const GlVertexArray::handleType GlVertexArray::handle() const
	{
		return this->_handle;
	}
